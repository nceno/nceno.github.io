...
/*  function getActivities(address _oracle, bytes _jobId, string _accessToken) public {
  ...
  function postRelayedCall(bytes calldata context, bool success, uint actualCharge, bytes calldata preRetVal) /*relayHubOnly*/ external {
  }
  ...
  modifier onlyAdmin(){
    require(getSender() == admin,"Sender not authorized."); //getSender()
    _;
  }
  ...
  function liquidateGoalAt(bytes calldata _goalID) onlyAdmin external {
    //can only call one week after a goal is finished
  ...
  function setPartitionChoices(uint[12][6][2] calldata _newChoices) onlyAdmin external {
...
/*  function getTestETH() public{
  ...
  bytes promoCode;
  uint promoCodeCount;
  function setPromoCode(bytes memory _code) public onlyAdmin{
    promoCode = _code;
  }
