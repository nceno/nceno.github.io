...
/*  function downloadVars() onlyAdmin public returns(address, uint, address, address, address, uint){
  ...
  function host(bytes32 _goalID, uint _activeMins,  uint _stakeUSD, uint _sesPerWk, uint _wks, uint _startTime, uint _stravaID, bytes32 _code)  external payable notHalted{
  ...
  function join(bytes32 _goalID, uint _stravaID, bytes32 _code)  external payable notHalted {
    //price eth internally using Kyber
    uint _ETHUSDprice;
    (_ETHUSDprice,) = proxy.getExpectedRate(ETH_ERC20, DAI_ERC20, msg.value);
    uint _ethPricePennies = _ETHUSDprice/10**16;
