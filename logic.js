  ...
  .call({from: web3.eth.defaultAccount},
    function(error, result) {
      if (!error){
        ...
        .send({from: web3.eth.defaultAccount, nonce: correctNonce, gas: 3000000, gasPrice: (standard+1)*1000000000, value: stakewei},
          function(error, result) {
            if (!error){
              $("#joinSearch").hide();
              $("#srCancelBtn").hide();
...
$("#claimBtn").click(function() {
  ...
  .send({from: web3.eth.defaultAccount, nonce: correctNonce, gas: 2000000, gasPrice: (standard+1)*1000000000},
    function(error, result) {
      if (!error){
        $("#claimBtn").hide();
        $("#claimLoader").show();
  ...
  xhr.addEventListener("readystatechange", function () {
    if (this.readyState === 4) {
      ...
      if(cleaned.length>0){
        ...
        .send({from: web3.eth.defaultAccount, nonce: correctNonce, gas: 6000000, gasPrice: (standard+1)*1000000000},
          function(error, result) {
            if (!error){
              /*$("#joinSearch").hide();
              $("#srCancelBtn").hide();
