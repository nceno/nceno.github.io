...
contract GoalFactory {
  ...
  mapping(bytes32 => competitorObject) profileOf; //registry of ALL users' info, indexed by wearableID
    mapping(bytes32 => bool) userExists; //to check if a user is registered with Nceno (used only when creating a new competitor)
  ...
  struct goalObject{
    ...
    mapping(bytes32 => bool[12]) bonusWasClaimed; //to check if a weekly bonus was claimed (claim)
  }
  goalObject[] allGoals; //iterable registry of all goals created, (only indexed by order of deployment).
    uint goalCount; //total number of goals
    mapping(bytes32 => goalObject) goalRegistry; //dictionary of goals, accessible by goalID
  ...
  function createGoal(bytes32 _goalID, uint _activeMins, uint _stakeWEI, uint _sesPerWk, uint _wks, uint256 _startTime, bytes32 _wearableID) external payable {
    ...
    //*createdGoal.isCompetitor[_wearableID] = true; //add self to competitor list exist
  ...
  function simplePayout(bytes32 _userID, uint _reportedMins, uint256 _timeStamp, bytes32 _goalID) external{
    ...
    require(goalRegistry[_goalID].isCompetitor[_userID]==true && (goalRegistry[_goalID].startTime < now < goalRegistry[_goalID].startTime + goalRegistry[_goalID].wks*1 weeks));
  ...
  function getSingleRev(bytes32 _goalID) external view returns (uint){
  ...
  function singleCashout(bytes32 _goalID) external{
    require(msg.sender == nceno);
    uint revenue;
    for(uint i =0; i<12; i++){
      revenue+= goalRegistry[_goalID].potWk[i];
  ...
  function getActiveRev() external view returns (uint){
    require(msg.sender == nceno);
    uint totalRev;
  ...
  function activeCashout() external{
    require(msg.sender == nceno);
    uint totalRev;
    for(uint j =0; j<goalCount; j++){
      if(now < allGoals[j].startTime + allGoals[j].wks*1 weeks){
  ...
  function getPastRev() external view returns (uint){
    require(msg.sender == nceno);
    uint pastRev;
    for(uint j =0; j<goalCount; j++){
      if(now > allGoals[j].startTime + allGoals[j].wks*1 weeks){
  ...
  function ncenoEmergencyCashout() external{
    require(msg.sender == nceno);
